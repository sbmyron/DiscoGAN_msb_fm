...
def get_data():
    ...
    elif args.task_name == 'piano2violin':
    	data_A, data_B = get_music_files(item = 'piano2violin', test = False)
    	test_A, test_B = get_music_files(item = 'piano2violin', test = True)
...
def main():
    ...
    if args.task_name.startswith('edges2'):
    	...
    	test_A = read_images_msb( test_style_A, 'A', args.image_size )
    	test_B = read_images_msb( test_style_B, 'B', args.image_size )
    ...
    if not load_pretrained:
        ...
        def best_finder(model_type, saved_models):
        ...
        gen_A = best_finder('gen_A', saved_models)
        gen_B = best_finder('gen_B', saved_models)
        dis_A = best_finder('dis_A', saved_models)
        dis_B = best_finder('dis_B', saved_models)
        print(gen_A, gen_B, dis_A, dis_B)

        generator_A = torch.load(model_path+'/'+gen_A)
        generator_B = torch.load(model_path+'/'+gen_B)
        discriminator_A = torch.load(model_path+'/'+dis_A)
        discriminator_B = torch.load(model_path+'/'+dis_B)
