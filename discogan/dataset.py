...
music_path = os.path.join(dataset_path, 'piano2violin')
...
def get_music_files(item='piano2violin', test=False, n_test=15): #msb 
    ...
    for i, file in enumerate(type_a_images):
        type_a_images[i] = image_paths[0]+'/'+file

    for i, file in enumerate(type_b_images):
        type_b_images[i] = image_paths[1]+'/'+file
 

    if test == False:
        return type_a_images[:-n_test], type_b_images[:-n_test]
    if test == True:
        return type_a_images[-n_test:], type_b_images[-n_test:]
